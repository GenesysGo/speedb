...
void ColumnFamilyData::SetL0BaseCompactionSpeed(uint64_t size) {
  ...
  lo_base_compaction_speed_ = size / l0_clearance_dur;
  started_l0_timer_ = false;
  l0_start_clearance_time_ = 0;
  ROCKS_LOG_WARN(ioptions_.logger,
                 "L0L1 compaction ended. duration : %" PRIu64
                 " . amount cleared: %" PRIu64,
                 l0_clearance_dur, size);
}
...
WriteStallCondition ColumnFamilyData::RecalculateWriteStallConditions(
  ...
  if (current_ != nullptr) {
    ...
    if (dynamic_delay) {
      // AutoTuneDelayedWriteRate();

      // reset time point when L0L1 finishes and calc speed.
      // when compaction ends, and set_l0_size_cleared is called, we calc the
      // rate.
